    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-6 text-center relative overflow-hidden font-sans">
        ...
        <div className="z-10 w-full max-w-sm flex flex-col items-center justify-center animate-in fade-in zoom-in duration-700">
          ...
          <div className="text-[10rem] md:text-[13rem] font-black text-red-600 italic leading-[0.7] drop-shadow-[0_0_50px_rgba(220,38,38,0.8)] mb-6 animate-micro [animation-delay:0.5s]">
          ...
          <div className="grid grid-cols-2 gap-3 w-full max-w-xs mb-16 relative z-50">
            <a href={`https://wa.me/?text=${encodeURIComponent(shareText + " " + shareUrl)}`} target="_blank" className="bg-[#25D366] text-white py-4 rounded-2xl flex items-center justify-center font-black text-xs uppercase shadow-lg active:scale-95 transition-all">WhatsApp</a>
            <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" className="bg-[#1877F2] text-white py-4 rounded-2xl flex items-center justify-center font-black text-xs uppercase shadow-lg active:scale-95 transition-all">Facebook</a>
            <button onClick={() => alert("¡Toma una captura de pantalla!")} className="bg-gradient-to-tr from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] text-white py-4 rounded-2xl font-black text-xs uppercase shadow-lg active:scale-95 transition-all">Instagram</button>
            <button onClick={() => {navigator.clipboard.writeText(shareUrl); alert("¡Copiado!");}} className="bg-zinc-800 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-lg active:scale-95 transition-all">Link</button>
          </div>
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col items-center">
        ...
        <div className="w-full bg-zinc-950/50 border border-zinc-900 p-8 rounded-[3rem] shadow-2xl backdrop-blur-sm">
          <div className="mb-10 text-left relative">
            ...
            <input type="text" value={searchKeyword} onChange={(e) => searchIdol(e.target.value)} className="w-full bg-zinc-900 border-2 border-zinc-800 p-5 rounded-2xl text-white text-lg outline-none focus:border-yellow-500 transition-all font-bold placeholder:text-zinc-700" placeholder="Escribe nombre..." />
          ...
          <div className="mb-10 text-left">
            ...
            <input type="text" value={userName} onChange={(e) => setUserName(e.target.value)} className="w-full bg-zinc-900 border-2 border-zinc-800 p-5 rounded-2xl text-white text-lg outline-none focus:border-yellow-500 transition-all font-bold" placeholder="Tu nombre" />
          </div>
          <div className="mb-12 text-left">
            ...
            <div className="flex gap-2">
              <select value={year} onChange={(e) => setYear(e.target.value)} className="flex-1 bg-zinc-900 border-2 border-zinc-800 p-4 rounded-xl text-white font-bold text-center appearance-none outline-none focus:border-yellow-500">{years.map(y => <option key={y} value={y}>{y}</option>)}</select>
              <select value={month} onChange={(e) => setMonth(e.target.value)} className="w-20 bg-zinc-900 border-2 border-zinc-800 p-4 rounded-xl text-white font-bold text-center appearance-none outline-none focus:border-yellow-500">{months.map(m => <option key={m} value={m}>{m}</option>)}</select>
